...
from game.constants import (
    ...
    RED, GREEN, WATER_BLUE, TURN_TIME, JEU_SOLO, FIN_PARTIE, CELL_SIZE, JEU_ONLINE
)
...
class GameScreen:
    ...
    # Modifications à apporter à ui/game_screen.py
    ...
    def draw(self, screen, background_drawer):
        ...
        if self.game_manager.is_online and self.game_manager.network_client:
            # On affiche directement le statut de connexion avec l'adversaire
            # sans utiliser l'attribut connection_status qui cause l'erreur
            connection_text = f"Connecté à {self.game_manager.opponent}"
            status_color = (50, 255, 100)  # Vert pour montrer que la connexion est active
        
            # Si on a un accès à l'attribut connection_status, on l'utilise
            if hasattr(self.game_manager.network_client, 'connection_status'):
                connection_text = self.game_manager.network_client.connection_status
                status_color = (50, 255, 100) if "Connecté" in connection_text else (255, 100, 100)
        
            status_text = self.small_font.render(f"État réseau: {connection_text}", True, status_color)
    ...
    def handle_event(self, event):
        ...
        if self.game_manager.is_message_active() or self.animated_text.is_active():
            return JEU_ONLINE if self.game_manager.is_online else JEU_SOLO
